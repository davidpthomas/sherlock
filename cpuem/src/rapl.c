...
static int get_os_map(APIC_ID_t *os_map, int os_cpu_count, int *max_pkg) {
...
static int construct_os_map(APIC_ID_t *os_map, long os_cpu_count, int *max_pkg) {
    if (get_os_map(os_map, os_cpu_count, max_pkg) != 0) {
        return -1;
    }
    return 0;
}
...
static int build_topology() {
    ...
    if (construct_os_map(os_map, os_cpu_count, &max_pkg) != 0) {
        return -1;
    }
